...
class TestMimetypeDOCX(UtilHelper):
    ...
    def test_send_docx_ok_result_99(self):
        tpt = self.get_tpt()
        ...
        data = self.get_sample_b64_from_path('document1.docx', mimetype)
        ...
        data = self.get_sample_b64_from_path('document2.docx', mimetype)
        request_id = tpt.task.prepare.execute(original_request_id, user_id, activity_id, data)
        tpt.task.extract.execute()
        tpt.task.compare.execute()
